...
public class ChooseJoinGameTypeAct extends Activity {

	GameBoardApplication _application;
	ArrayList<IGameProxy> _gameProxiesList = new ArrayList<IGameProxy>();
	Context _currentContext;
	Map<Button, Class<?>> _buttonProxyClassMap = new HashMap<Button, Class<?>>();

	@Override
	public void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.choosejoingametype);
	...
	View.OnClickListener getOnClickCreateConfigurator(final Button button) {
		return new View.OnClickListener() {
			public void onClick(View v) {
				...
				Intent i = new Intent(_currentContext, OpenedGamesListAct.class);
				i.putExtra("gameProxyClass", _buttonProxyClassMap.get(button));
				startActivity(i);
			}
		};
	}
	...
	private Button CreateGameButton(IGameProxy proxy) {
		...
		_buttonProxyClassMap.put(newGameButton, proxy.getClass());
